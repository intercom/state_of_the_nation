module StateOfTheNation
  class QueryString
    ...
    def self.database_appropriate_types(klass)
      return {
        postgresql: {
          active_scope: "(%{finish_key} IS NULL OR %{finish_key} > ?::timestamp) AND (%{start_key} IS NULL OR %{start_key} <= ?::timestamp)",
          less_than: "(%{start_key} IS NULL OR %{start_key} < ?::timestamp)",
          greater_than_or_null: "(%{finish_key} > ?::timestamp) OR (%{finish_key} IS NULL)",
        },
        mysql: {
          active_scope: "(%{finish_key} IS NULL OR %{finish_key} > ?) AND (%{start_key} IS NULL OR %{start_key} <= ?)",
          less_than: "(%{start_key} IS NULL OR %{start_key} < ?)",
